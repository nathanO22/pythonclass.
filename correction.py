...
import random
with open("bank_d.txt", "r") as file:
    data =eval(file.read())

with open("log.txt", "r") as fi:
    log =eval(fi.read())
...
def withdraw(user_detail:dict):
    ...
    if trans_pin == user_detail["transaction_pin"]:
        ...
        if user_detail["balance"] > amount:
            ...
            transaction = f"you withdrew {amount}"
            for k, v in data:
                user_detail["acc_num"] : transaction
...
def transfer(user_detail:dict, data:dict):
    ...
    if beneficiary_detail:
        if user_detail["balance"] > amount:
            ...
            transaction = f"you transferred {amount} to {beneficiary}"
            # for k, v in data:
            #     user_detail["acc_num"] : transaction
            #     with open("log.txt", "w") as fi:
            #         fi.write(k,v)
    ...
    if choice == "s":
        ...
        if user_detail:
            while True:
                ...
                with open("bank_d.txt", "w") as file:
                    file.write(str(data))  
